...
def run():
	...
	# response = requests.get(requestUrl + apiKey)
		...
		if(previous is None):
			...
			if(game["subType"] == "RANKED_SOLO_5x5"):
				...
				if(game["teamId"] == 100):
					...
					levelTeam1.append(rank_stats(summonerId, c))
				else:
					champsTeam2.append(game["championId"])
					levelTeam2.append(rank_stats(summonerId, c))

				for player in game["fellowPlayers"] :
					if(player["teamId"] == 100):
						champsTeam1.append(player["championId"])
						levelTeam1.append(rank_stats(str(player["summonerId"]), c))
					else :
						champsTeam2.append(player["championId"])
						levelTeam2.append(rank_stats(str(player["summonerId"]), c))
